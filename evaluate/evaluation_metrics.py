...
log_file = 'experiments-final-2.jsonl'
logs = []
data = []
...
guideline = """ 
          You are given the following background texts about the fictional country of Tuberlinlandia:
            ...
            Inland page: 
            The main article on the inland page is about Minister Paul Schwertz Holds Secret Meeting with Controversial Vivrath Math Club.
            ...
            Keywords: "Vivrath Mathematical Society", "AI Regulation", "Deltasync AI", "Tuberlinlandia"
            ...
            Topic: Culture / Arts
            Alternative Title: Ancient Sounds, New Voices
            Body:
            ...
            Topic:
            Science, Ethics, Investigation
            Alternative Title: GeneFab Scandal
            Body:
            ...
            Please rate, on a scale from 0.0 (completely incorrect) to 1.0 (fully correct), how accurately the answer reflects the facts stated in the background texts. Provide only a single numeric score."""

# Metrics to evaluate

guideline_response_completeness = """Measure whether the answer covers all aspects of the user’s question based on the website content. " 
    "If the response states that “the website does not provide this information” (or equivalent), treat that aspect as missing—do *not* count the disclaimer itself as coverage. "
    "Please rate, on a scale from 0.0 (completely incomplete) to 1.0 (fully complete), how thoroughly the answer addresses every required element. """

guideline_response_relevance = """Measure whether the answer stays strictly on-topic and addresses the user’s question without adding any unrelated information. "
    "If the response states “the website does not provide this information” (or equivalent), treat that as off-topic and score it as completely irrelevant. "
    "Please rate, on a scale from 0.0 (completely irrelevant) to 1.0 (fully relevant), how well the response focuses only on the requested content and excludes all extraneous details. """


metrics = [
    GuidelineAdherence(guideline=guideline_response_completeness, guideline_name="response_completeness"),
    GuidelineAdherence(guideline=guideline_response_relevance, guideline_name="response_relevance"),
...
csv_file = 'evaluation_results_final-2.csv'
with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
    ...
    for entry, row in zip(logs, results):
        writer.writerow({
            ...
            "score_response_completeness": row.get("score_response_completeness_adherence", ""),
            "reasoning_response_completeness": extract_reasoning(row, "explanation_response_completeness_adherence"),
            "score_response_relevance": row.get("score_response_relevance_adherence", ""),
            "reasoning_response_relevance": extract_reasoning(row, "explanation_response_relevance_adherence"),
